...
2. **トレーナー紹介(3名)** — 実写真・氏名・紹介文に差し替え(現在は仮名「◯◯ ◯◯」+ドラフト文)
3. **お客様の声** — 仮の声を掲載中。掲載許可済みの実際の声に差し替え
4. **駅からの徒歩分数** — 恵比寿店・小岩店それぞれの「徒歩◯分」を実測値に
5. **体験の所要時間** — 体験の流れセクション(現在は時間の記載なし)
6. **GA4測定ID** — `js/script.js` の `GA_MEASUREMENT_ID` に設定(空のままなら計測なしで動作)
7. **OGP画像** — 現在は小岩店写真を流用。専用画像(1200x630)制作後に差し替え
8. **favicon / apple-touch-icon** — 暫定のデータURI faviconを正式版に差し替え

※ 料金プラン(定額+回数券)・2店舗の住所・JSON-LDは実データ反映済み(2026-07-02)
